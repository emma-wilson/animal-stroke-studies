...
dedup_labelled_step  <-function(x,
  ...
  newdatformatted <- x  %>%
    select(Author, Title, Year, Journal, Abstract, Number, Pages, Volume, RecordID, Label) 
  ...
  ##Get rid of punctuation and differnces in formatting
  ...
  # ROUND 1: run compare.dedup function and block by Title&Pages OR Title&Author OR Title&Abstract 
  newpairs = compare.dedup(newdatformatted, blockfld = list(c(2,8), c(1,2), c(2,5)), strcmp = TRUE, exclude=c("RecordID", "Label"))
  ...
  newpairs2 = compare.dedup(newdatformatted, blockfld = list(c(1,3,8), c(4,9,8), c(10,9,8)), strcmp = TRUE, exclude= c("RecordID", "Label"))
  ...
  SeePairs <- SeePairs %>%
    select(id1, id2, Author1, Author2, Author, Title1, Title2, Title, Abstract1, Abstract2, Abstract, Year1, Year2, Year, Number1, Number2, Number, Pages1, Pages2, Pages, Volume1, Volume2, Volume, Journal1, Journal2, Journal, RecordID1, RecordID2, Label1, Label2)
  ...
  # Add in problem matching pairs and different year data in ManualDedup
